    ...
    return (
        <Link to={`/node/${id}`} className="block">
            <motion.div
                ...
                className={`flex flex-col lg:grid lg:grid-cols-12 gap-10 lg:gap-20 items-start py-12 md:py-24 lg:py-40 px-6 sm:px-12 md:px-24 group/project hover:bg-white/[0.03] transition-all duration-700 relative ${!isLast ? 'border-b border-white/5' : ''}`}
            >
                <div className="lg:col-span-8 space-y-16 relative z-10">
                    <div className="flex items-center gap-6">
                        ...
                        <div className="h-[1px] w-12 bg-white/5 group-hover/project:w-20 transition-all duration-700"></div>
                    </div>

                    <h3 className="font-body font-semibold text-2xl sm:text-4xl lg:text-5xl text-white tracking-[-0.06em] leading-[1.1]">
                        {title}
                    </h3>

                    <p className="font-body text-base sm:text-lg md:text-xl text-white/40 leading-snug max-w-3xl italic group-hover/project:text-white/60 transition-colors">
                        {description}
                    </p>

                    <div className="flex flex-wrap gap-4 pt-4">
                        {tags.map(t => (
                            <span key={t} className="px-6 py-2 bg-white/5 border border-white/5 font-body text-[11px] text-white/50 uppercase tracking-widest rounded-full group-hover/project:border-primary/20 transition-colors">{t}</span>
                        ))}
                    </div>
                </div>
                ...
                <div className="lg:col-span-4 flex flex-col justify-end items-start lg:items-end text-left lg:text-right space-y-6 self-center">
                    ...
                    <div className="flex flex-col items-start lg:items-end">
                        <span className="font-body font-bold text-4xl sm:text-5xl md:text-7xl text-white tracking-tighter leading-none tabular-nums drop-shadow-lg">{metric}</span>
                        <span className="font-body text-xs text-white/20 uppercase tracking-[0.4em] mt-3 italic">{metricLabel}</span>
                    </div>
                </div>

                {/* Hover Indicator */}
                <div className="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-0 bg-primary group-hover/project:h-1/2 transition-all duration-700" />
            </motion.div>
        </Link>
    );
};
